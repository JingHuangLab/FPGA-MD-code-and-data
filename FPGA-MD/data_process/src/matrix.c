...
static Matrix *_subMatrix(Matrix *M, int r, int c){
    Matrix *re = M_Zeros(M->row-1,M->column-1);
    int i,j,k=0;
    for(i=0;i<M->row;++i){
        if(i==r)
            continue;
 ...
 * 行列式*/
    ...
    if (_mat_->column == _mat_->row) {
        ...
        */
       if(_mat_->row == 1){
           _det_mat = _mat_->data[0];
       }
       else if(_mat_->row == 2){
           _det_mat = _mat_->data[0]*_mat_->data[3] - _mat_->data[1]*_mat_->data[2] ;
       }
       else{
           for(int j = 0; j<_mat_->column;j++){
               if(_mat_->data[index(_mat_,0,j)]==0){
                   _det_mat +=0;
               }
               else{
                   Matrix *sub_matrix = _subMatrix(_mat_,0,j);
                   _det_mat +=pow(-1,j+1+1)*_mat_->data[index(_mat_,0,j)]*M_det(sub_matrix);
                   M_free(sub_matrix);
               }
           }
       }
