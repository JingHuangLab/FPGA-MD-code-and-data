...
if {![string equal [version -short] $VIVADO_VERSION]} {
    puts "MD base project requires Vivado version $VIVADO_VERSION"
    #exit
}
...
set ip_repo_path ${root_dir}/src/

set open_nic_shell md_base
...
array set build_options {
    -board_repo  "../board_files/f37x/1.3/"
    -board       f37x
    -tag         "RLv1"
    -overwrite   0
    -jobs        8
...
set board_part inspur.com:f37x:part0:1.3
set zynq_family 0
...
dict append module_dict RL_v1               "${src_dir}/RL_v1"
#dict append module_dict cmac_subsystem      "${src_dir}/cmac_subsystem"
...
# Setup vivado_ip directory for the design
set ip_build_dir ${build_dir}/vivado_ip
if {![file exists ${ip_build_dir}/]} {
...
dict for {module module_dir} $module_dict {
    ...
    foreach ip $ips {
        ...
        if {[file exists "${ip_tcl_dir}/${ip}.tcl"]} {
            source ${ip_tcl_dir}/${ip}.tcl
        } elseif {[file exists "${ip_tcl_dir}/${ip}/${ip}.xci"]} {
            #read_ip ${ip_tcl_dir}/${ip}/${ip}.xci
            import_files ${ip_tcl_dir}/${ip}/${ip}.xci
        } else {
            continue
        }
        ...
        if {$synth_ip} {
            ...
            lappend ip_synth_str ${ip}_synth_1
...
#lappend include_dirs "${src_dir}/include"
...
# IP Synthesis launch_runs
if {$synth_ip} {
    puts "INFO: IP Synthesis launch_runs -jobs $jobs $ip_synth_str "
    launch_runs -jobs $jobs $ip_synth_str
    foreach ip_syn $ip_synth_str {
        wait_on_run $ip_syn
    }
}

# build xdma-subsystem
set bdName xdma
source ${src_dir}/xdma_subsystem/xdma.tcl
make_wrapper -files [get_files ${top_build_dir}/${top}.srcs/sources_1/bd/${bdName}/${bdName}.bd] -top
add_files -norecurse ${top_build_dir}/${top}.gen/sources_1/bd/${bdName}/hdl/${bdName}_wrapper.v
source ${src_dir}/xdma_subsystem/bram.tcl

# Read top-level source files
#read_verilog -quiet     [glob -nocomplain -directory $src_dir "open_nic_shell_macros.vh"]
read_verilog -quiet -sv [glob -nocomplain -directory $src_dir "md_base_top.sv"]
#read_vhdl -quiet [glob -nocomplain -directory $src_dir "*.vhd"]


#import_files -quiet ${src_dir}/dma_tar_axi_cross.xci
#upgrade_ip [get_ips dma_tar_axi_cross]
#generate_target all [get_ips dma_tar_axi_cross]
...
set_property top md_base_top [get_property srcset [current_run]]

#source $script_dir/add_debug_ila.tcl

# Read constraint files
read_xdc -unmanaged ${constr_dir}/${board}/pins_md.xdc
#read_xdc -unmanaged ${constr_dir}/${board}/timing.xdc
read_xdc            ${constr_dir}/${board}/general.xdc
set_property target_constrs_file ${constr_dir}/${board}/general.xdc [current_fileset -constrset]
set_property used_in_synthesis false [get_files  ${constr_dir}/${board}/pins_md.xdc]
